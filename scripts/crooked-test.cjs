...
const ЭКРАНЫ = [
  ...
  // v1.563.0: СВЕТЛАЯ ТЕМА. Её здесь не было ни одной строкой — все шестнадцать
  // экранов выше тёмные, и «всего кривостей: 0» относилось только к тёмной.
  //
  // Хуже того: до этой же версии светлая тема на стенде вообще НЕ ВКЛЮЧАЛАСЬ —
  // SettingsProvider затирал признак темы сохранённой настройкой (починено в
  // src/components/__site_shots.tsx). То есть строки эти нельзя было завести
  // раньше при всём желании: страница light.html рисовалась тёмной, и проверка
  // проверяла бы дважды одно и то же, честно отвечая «чисто».
  //
  // Светлая ломается чаще тёмной, и по понятной причине: подложки и разделители
  // в приложении задавались белым полупрозрачным (см. --ov в lib/settings.tsx),
  // а белое по белому исчезает.
  { имя: 'СВЕТЛАЯ: сервер, 1440',      стр: 'light', ш: 1440, в: 900, дела: ['сервер'] },
  { имя: 'СВЕТЛАЯ: ЛС, 1440',          стр: 'light', ш: 1440, в: 900, дела: [] },
  { имя: 'СВЕТЛАЯ: сервер, 1000',      стр: 'light', ш: 1000, в: 760, дела: ['сервер'] },
  { имя: 'СВЕТЛАЯ: телефон, 412',      стр: 'light', ш: 412,  в: 892, дела: ['сервер'] },
  { имя: 'СВЕТЛАЯ: настройки, 1440',   стр: 'light', ш: 1440, в: 900, дела: ['настройки'] },
  { имя: 'СВЕТЛАЯ: настройки, окно 760', стр: 'light', ш: 760, в: 560, дела: ['настройки'] },
  { имя: 'СВЕТЛАЯ: меню у угла',       стр: 'light', ш: 1440, в: 900, дела: ['сервер', 'меню-угол'] },
  { имя: 'СВЕТЛАЯ: длинные имена, 760', стр: 'light', ш: 760, в: 560, дела: ['сервер'], длинно: true },
  { имя: 'СВЕТЛАЯ: крошечное окно 420×360', стр: 'light', ш: 420, в: 360, дела: ['сервер'] },
]
