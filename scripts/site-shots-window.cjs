...
const СНИМКИ = [
  ...
  { имя: 'phone-server',   ш: 412,  в: 892, тема: 'dark',  дела: ['сервер', 'закрыть-участников'] },
]
...
const ДЕЛА = {
  ...
  // v1.563.0: на снимке телефона список участников надо ЗАКРЫТЬ.
  //
  // На настоящем телефоне он закрыт с самого начала — ServerView.tsx:202,
  // `IS_MOBILE ? false : …`. Но IS_MOBILE считается по устройству, а не по
  // ширине окна, и в Electron он ложный: стенд шёл по настольной ветке и
  // открывал панель. На 412 точках она накрывала переписку больше чем
  // наполовину, обрезая текст сообщений на полуслове — и ровно в таком виде
  // снимок лежал на официальном сайте как «вид с телефона». Показывали то,
  // чего на телефоне не бывает.
  //
  // Закрываем настоящей кнопкой, а не подменой признака: этот файл держится
  // правила «снимок обязан показывать то, до чего человек доходит руками».
  'закрыть-участников': [НАЖАТЬ('.ph2-btns .pin-btn[title="Скрыть участников"]')],
