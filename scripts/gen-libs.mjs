...
const БИБЛИОТЕКИ = [
  {
    id: 'three',
    пакет: 'three',
    // v1.555.0: загрузчики моделей едут ВМЕСТЕ с three.
    //
    // Пакет three отдаёт из главного модуля только ядро — GLTFLoader'а там нет,
    // он лежит отдельным файлом в examples/jsm. Без него мастерская умела
    // ровно шесть примитивов: куб, шар, цилиндр, плоскость, свет, пустышку. Ни
    // одной готовой модели в сцену было не положить, а «сделать 3д модели»
    // ровно про это.
    //
    // Почему именно эти два и почему не Draco. glTF — обычный формат обмена
    // (Blender, Unity, Sketchfab отдают его как есть), OBJ — то, во что
    // упирается всё старое. DRACOLoader сюда не годится принципиально: он
    // тянет распаковщик отдельным файлом с чужого адреса, а страница плагина
    // живёт в песочнице без сети и без доступа к нашим файлам.
    ещё: [
      { имя: 'GLTFLoader', путь: 'three/examples/jsm/loaders/GLTFLoader.js' },
      { имя: 'OBJLoader', путь: 'three/examples/jsm/loaders/OBJLoader.js' },
    ],
    global: 'THREE',
    описание: 'Трёхмерная графика: сцены, камеры, материалы, загрузчики моделей (GLTFLoader, OBJLoader)',
    тип: 'module',
  },
]
...
for (const b of БИБЛИОТЕКИ) {
  ...
  // Дополнения реэкспортируем ПОИМЁННО, а не звёздочкой: звёздочка при
  // совпадении имён молча выбрасывает обе стороны, и понять, почему пропал
  // кусок библиотеки, было бы нечем.
  const строки = ['export * from ' + JSON.stringify(b.пакет) + ';']
  for (const д of b.ещё ?? []) {
    строки.push('export { ' + д.имя + ' } from ' + JSON.stringify(д.путь) + ';')
  }
  writeFileSync(вход, строки.join('\n'))
  try {
