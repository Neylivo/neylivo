...
//
// Чего не было. NeyLivo Music играл только пока приложение открыто и на экране: ни
...
//   • Браузер и PWA: в шторке, на экране блокировки и на клавиатуре ноутбука
//     появляется карточка с обложкой и кнопками.
//   • Десктоп (Electron): системный виджет мультимедиа Windows.
//   • Android, наше приложение — НИЧЕГО. Здесь v1.502.0 исправляет то, что
//     раньше было написано в этом же комментарии неверно. Страница живёт в
//     WebView, а WebView системной карточки не показывает: navigator.mediaSession
//     там принимает данные и молчит. Владелец видел вместо карточки наше
//     служебное уведомление и сказал об этом прямо. Карточку на Android теперь
//     рисует своя служба с настоящим android.media.session.MediaSession — см.
//     src/lib/keepAlive.ts и MusicService.java. Действия у обоих путей общие
//     (mediaRef в MusicPlayer.tsx), иначе они разошлись бы.
//
...
export function setMediaNow(now: MediaNow | null) {
  ...
  if (key !== lastKey) {
    ...
    try {
      ...
      if (MM) {
        ms.metadata = new MM({
          title: now.title || 'NeyLivo Music',
          artist: now.artist || '',
          album: now.album || 'NeyLivo Music',
