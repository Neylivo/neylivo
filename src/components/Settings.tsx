...
const NAV: { group: string | null; items: { k: string; label: string; icon: string }[] }[] = [
  ...
  // «Разработчикам», хотя это одно и то же дело: взять готовое из каталога или
  // сделать своё. Плюс «Разработчикам» отпугивало ровно тех, кому каталог и
...
function E2eeSection() {
  ...
  return <>
    ...
    {/* v1.385.0: вложения отделены от текста и по умолчанию выключены. Файл
        уезжает шифротекстом, и если расшифровать его не удалось, человек видит
        не картинку, а «это не картинка» — то есть теряет фото. Текст при этом
        шифруется исправно, поэтому его отключать незачем. */}
    <Row title="Шифровать вложения в личке"
      desc="Фото и файлы в переписке один-на-один. По умолчанию выключено: зашифрованное вложение, которое не удалось открыть, показывается как повреждённое, и фото теряется. Текст сообщений шифруется в любом случае.">
      <Toggle on={settings.e2eeFiles} onChange={v => set('e2eeFiles', v)} />
    </Row>
    <Row title="Шифровать разговор в звонках"
            ...
            <div className="pqs2-inner" key={cat}>
              ...
              {cat === 'privacy' && <>
                ...
                <div className="pqs2-desc" style={{ marginTop: 10 }}>
                  ...
                  ни отчётов об ошибках на сторону в нём нет. Сервер видит только то, без чего невозможно
                  доставить сообщение: кто кому пишет и когда.
                </div>
