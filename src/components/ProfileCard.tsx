...
export type WidgetField = 'favGames' | 'wishGames'
...
function WidgetTile({ label, games, covers, isMe, onClick }:
  ...
  return (
    <div className={'pc-widget' + (cover ? ' has-cover' : '')}
...
export function ProfileCard({ userId, name, avatarUrl, status, onClose, initialTab = 'board' }:
  ...
  // тот же общий кэш обложек, что и у «Недавней активности».
  useEffect(() => {
    let ok = true
    for (const n of [...favs, ...wish]) if (n) resolveCover(n).then(u => { if (ok) setWidgetCovers(c => ({ ...c, [n]: u })) })
    return () => { ok = false }
  ...
  const WIDGET_MAX = 8   // v1.171.0: «Мои любимые игры» / «Хочу поиграть» — до 8 игр, как в Discord
  ...
  function toggleMulti(field: WidgetField, g: string) {
    const head = field === 'favGames' && pp.favGames[0] ? [pp.favGames[0]] : []
    const rest = field === 'favGames' ? pp.favGames.slice(1) : pp[field]
    const nextRest = rest.includes(g) ? rest.filter(x => x !== g) : (rest.length < WIDGET_MAX ? [...rest, g] : rest)
    saveWidgetField(field, [...head, ...nextRest])
  }
  ...
  function pickFav(g: string) {
    saveWidgetField('favGames', [g, ...pp.favGames.slice(1)])
    setGamePicker(null)
  }
  ...
  return (<>
    <div className="pc-backdrop" onClick={onClose}>
      ...
      {gamePicker?.mode === 'single' && <GamePickerModal title="Любимая игра" onPick={pickFav} onClose={() => setGamePicker(null)} />}
      {gamePicker?.mode === 'multi' && <WidgetGamesModal
