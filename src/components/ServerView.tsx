...
import { mentionsUser, mentionsRoleName, mentionsHere } from '../lib/mentions'
...
export function ServerView({ server, username, avatarUrl, onAvatar, onLeft }:
  ...
  async function editMsg(id: string, content: string) {
    ...
    if (hasInterceptors('send')) {
      const r = await runBeforeSend(content, curChannel?.id ?? null, (pid, fn, a) => invokePlugin(pid, fn, a))
  ...
  function verifyBlockReason(): string | null {
    ...
    if (lvl >= 4 && !u.phone_confirmed_at)
      return 'Владелец разрешил писать только с подтверждённым номером телефона. Вход по телефону в NeyLivo пока не настроен, так что написать смогут только владелец и те, кому выдана роль.'
    return null
  }
