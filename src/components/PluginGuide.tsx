...
const TABS: { id: Tab; label: string; icon: string }[] = [
  ...
  { id: 'rules', label: 'Правила', icon: 'list' },
]
        ...
        <div className="pg-body">
          ...
          <div className="pg-main"><div className="pg-inner">
            ...
            {tab === 'rules' && <>
              ...
              <h3>Своё окно с настоящей страницей</h3>
              <p>
                Плагин может открыть окно, внутри которого обычная браузерная страница —
                свой DOM, свои стили, свой код. Это не «панель со строчками», а полноценное
                место, где живут игра, редактор или инструмент.
              </p>
              <div className="pg-tbl">
                <div><code>canvas</code><span>2d, webgl, webgl2 и WebGPU — настоящие</span></div>
                <div><code>кадры</code><span>requestAnimationFrame, замерено 124 в секунду</span></div>
                <div><code>мышь</code><span>движение, кнопки, колесо, захват указателя</span></div>
                <div><code>клавиатура</code><span>keydown и keyup, касания</span></div>
                <div><code>звук</code><span>AudioContext и свои файлы</span></div>
                <div><code>файлы</code><span>открыть, сохранить, перетащить в окно</span></div>
                <div><code>библиотеки</code><span>three.js встроен, любые другие — своим файлом</span></div>
              </div>
              <p className="pg-note">
                Проще всего начать не с кода, а с конструктора: Настройки → Плагины →
                <b> Приложения</b>. Там три поля (разметка, стили, код), живой показ рядом
                и готовые заготовки — трёхмерная сцена, игра на холсте, работа с файлами.
              </p>

              <h3>Пределы</h3>
              <p>
                Их нет. Ни на частоту, ни на количество, ни на размер: сколько сообщений
                отправить, сколько завести кнопок, команд, окон и фоновых задач, какого
                размера складывать файлы — решает плагин, а не приложение.
              </p>
              <p className="pg-note">
                Обратная сторона: отвечать за то, что плагин сделает от твоего имени,
                будешь ты. Плагин с ошибкой в цикле может завалить чат сообщениями, и
                выглядеть это будет как твои сообщения. Ставь то, чему доверяешь.
              </p>
              <p>
                Осталось ровно одно, и это не про «сколько можно», а про то, чтобы
                зависший плагин не утащил за собой приложение: если он не отвечает,
                приложение ждёт его до минуты на запуске и до десяти минут на вызов,
                а потом живёт дальше без него. Плюс запуск с зажатым Shift отключает
                все сторонние плагины разом.
              </p>
