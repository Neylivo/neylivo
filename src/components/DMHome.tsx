...
import { joinRoom, RoomEvent } from '../lib/livekit'
...
export function DMHome({ username, handle, avatarUrl, onAvatar, servers }:
  ...
  function finishCallMsg() {
    ...
    supabase.from('dm_messages').update({ content }).eq('id', id).then(({ error }) => { if (error) logErr('call msg] update', error) })
  }
  ...
  async function addByName() {
    // v1.402.0: хвост «#1234» отрезаем и ищем по одному имени. Юзернейм
    // уникален — второй способ назвать человека не нужен, а привычка дописывать
    // решётку у людей осталась, и спотыкаться о неё они не должны.
