    ...
    <div className="cset sset">
      ...
      <div className="cset-main">
        ...
        {tab === 'engage' && <>
          ...
          {toggleRow('Писать в канал, когда кто-то присоединился к серверу', 'Короткая строчка «X присоединился к серверу». Пишет сама база при вступлении, поэтому появляется и когда человек вошёл с другого устройства.', 'sys_welcome', true)}
        ...
        {tab === 'access' && <>
          ...
          {/* v1.324.0: «Публичный» стал настоящим. Раньше здесь был выбор из трёх
              способов вступления, ни один из которых ни на что не влиял, а в
              v1.321.0 я заменил его строкой «сервер не показывается в поиске» —
              и это оказалось неправдой: миграция 76 открыла чтение таблицы
              серверов всем вошедшим, поэтому в «Путешествии по серверам» лежали
              вообще все серверы подряд, включая чужие и служебные. Починено в
              supabase/84_public_servers.sql, а переключатель ниже — то, что этой
              видимостью управляет. */}
          <div className="cset-h" style={{ fontSize: 16, marginBottom: 4 }}>Как можно присоединиться к серверу</div>
          <div className="cset-row" style={{ marginTop: 4 }}>
            <div>
              <div className="cset-row-t">Публичный сервер</div>
              <div className="cset-hint">
                {st.public
                  ? 'Сервер виден в «Путешествии по серверам» — найти по названию и войти может любой, без приглашения.'
                  : 'Сейчас сервер закрыт: его не видно в поиске, и попасть можно только по ссылке-приглашению. Проверяется базой — чужой не прочитает даже название.'}
              </div>
            </div>
            <button className={'tgl' + (st.public ? ' on' : '')} onClick={() => up('public', !st.public)} />
          </div>
          {st.public && <div className="cset-hint" style={{ background: 'rgba(237,66,69,.12)', border: '1px solid rgba(237,66,69,.35)', borderRadius: 8, padding: '8px 10px' }}>
            ⚠️ Название, значок и описание сервера станут видны всем. Бан и приостановка приглашений при этом продолжают работать — забаненный не войдёт.
          </div>}
          <div className="cset-div" />
