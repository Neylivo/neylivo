...
export const THEMES: ThemeDef[] = [
  { key:'dark',     name:'NeyLivo Dark',  dark:'#1e1f22', main:'#23272a', panel:'#2b2d31', content:'#313338', hover:'#383a40', active:'#35373c', accent:'#5865f2', tx:'#dbdee1', mut:'#949ba4' },
...
function apply(s: Settings) {
  ...
  // v1.513.0: подсветка строки ввода при наборе — только если её включили.
  // Признак ставится на корень страницы, и решают стили: правило без признака
  // гасит кольцо у всех пяти видов поля разом. Иначе пришлось бы дублировать
  // отмену для каждого вида, и новый вид принёс бы кольцо обратно молча.
  if (s.focusRing) root.setAttribute('data-focusring', '1')
  else root.removeAttribute('data-focusring')
  // v1.467.0: плагины узнают о смене темы.
  //
  // Без этого плагин, нарисовавший что-нибудь своими цветами (холст, картинка,
  // подобранный под тему оттенок), после переключения на светлую тему оставался
  // тёмным пятном — и выглядело это как «плагин сломался». Спрашивать тему по
  // таймеру он не должен: ровно от этого и заводились события.
  //
  // Шлём ПОСЛЕ применения: плагин, спросивший в обработчике текущие цвета,
  // должен увидеть новые, а не те, что были секунду назад.
  // Развязка нарочная: этот файл — сердце настроек, и тянуть в него всю систему
  // плагинов ради одного события значило бы утащить её в стартовую сборку.
  // Событие окна ловит App.tsx и передаёт плагинам — там они уже есть.
  try {
    window.dispatchEvent(new CustomEvent('ponoi-theme', {
      detail: { theme: day ? 'light' : s.theme, light, accent: root.style.getPropertyValue('--c-accent').trim() },
    }))
  } catch { /* нет окна — значит и плагинов нет */ }
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  //
  // Понадобилось из-за плагинов: neylivo.status.set пишет ту же строку активности,
