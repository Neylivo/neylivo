...
import { публичный } from './attachUrl'
...
// v1.557.0: копируется ПОСТОЯННЫЙ адрес, а не подписанный. Показ вложений
// перешёл на подписанные ссылки (см. lib/attachUrl.ts), а они живут час —
// человек, который копирует ссылку, кладёт её в заметку или в другой чат и
// ждёт ссылку, а не тыкву.
export async function copyMediaLink(url: string): Promise<void> {
  await copyText(публичный(url.replace('#spoiler', '')), 'Ссылка скопирована')
}
