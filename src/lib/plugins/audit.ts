...
export function installRisks(
  ...
  // v1.486.0: у НАШИХ плагинов рисков не показываем.
  //
  // Владелец справедливо возмутился: «почему красным от наших мигают, они офиц».
  // И правда: список рисков считается по разрешениям, а наши плагины просят
  // ровно то, без чего не работают, — «Чистка фотографий» без доступа к файлам
  // не почистит ничего. Красная строка тут пугает там, где бояться нечего:
  // код этих плагинов идёт в самой сборке, его писали мы.
  //
  // Что при этом НЕ меняется: разрешения человек всё равно видит списком, а
  // «наш» опознаётся по коду (official.ts), а не по имени в шапке.
  if (official) return []
