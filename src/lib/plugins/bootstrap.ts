...
const ponoi = {
  ...
  apps: {
    ...
    // v1.487.0: спрятать окно и показать обратно. Это НЕ закрытие: содержимое
    // и холст остаются живыми, номер окна прежний. Нужно, например, чтобы
    // слить два окна в одно и вернуть обратно, не начиная рисовать заново.
    hide: (id) => call('apps.hide', [Number(id)]),
    show: (id) => call('apps.show', [Number(id)]),
  },
  // v1.492.0: встроенные библиотеки. В потоке от них толку немного — выполнять
  // их негде, DOM тут нет, — но СПИСОК и ТЕКСТ доступны и отсюда: плагин может
  // сложить нужное в html своего окна ещё до его открытия.
  libs: {
    list: () => call('libs.list', []),
