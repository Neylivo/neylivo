...
ЕСЛИ @permissions НЕ НАПИСАТЬ ВОВСЕ — плагин получит ВСЕ разрешения, и человек
увидит это на установке всеми красными строками сразу. То есть забытая строка
больше не роняет плагин на первом вызове.

Написал список — действует он: всё, чего в нём нет, отвечает внятной ошибкой, а
не молчит. Перечисляй ровно то, что используешь: лишние разрешения человек
видит при установке, и лишнее его отпугнёт. Чтобы честно сказать «мне не нужно
ничего», напиши: @permissions none

Плагин, собранный ТОБОЙ в конструкторе или мастерской, разрешений не
спрашивает вовсе: это твой собственный код, и согласия у себя не спрашивают.
...
Формат .glb — то, что нужно: у него меши, материалы и текстуры лежат ВНУТРИ
одного файла. Формат .gltf хранит их россыпью рядом, и приедет к тебе пустая
оболочка. Сжатие Draco не поддерживается: распаковщик к нему — отдельный файл с
чужого адреса, а сети у страницы нет.
