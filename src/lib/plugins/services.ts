 ...
 * Завести службу.
 *
 * v1.499.0: занятое имя больше НЕ отказ. Раньше второй плагин получал ошибку, и
 * его автор ничего не мог с этим сделать: узнать заранее, какие имена занял
 * чужой плагин, ему нечем. Теперь имя переходит к последнему — но НЕ молча:
 * прежний владелец видит это в своём журнале (takenFrom ниже читает api.ts).
 */
/** У кого забрали имя при последней перерегистрации. Читает api.ts. */
let отобрано: string | null = null
export function takenFrom(): string | null { const x = отобрано; отобрано = null; return x }
...
export function registerService(pluginId: string, name: string, methods: Map<string, FnRef>): Service {
  ...
  if (было && было.pluginId !== pluginId) {
    // v1.499.0: перебиваем, а не отказываем — по той же причине, что и с
    // командами: занятое чужим имя не должно ронять твой плагин. Узнать
    // заранее, что имя кем-то занято, автору нечем.
    //
    // Но НЕ молча: кто у кого забрал, обязано быть видно. Сообщает об этом
    // тот, кто нас позвал (api.ts), — здесь мы только отдаём прежнего
    // владельца наружу.
    отобрано = было.pluginId
    services.delete(n)
  }
