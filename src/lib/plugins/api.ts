...
export const PLUGIN_METHODS = [
  ...
  'apps.hide', 'apps.show',
  // v1.492.0: встроенные библиотеки для страницы плагина. Разрешения не
  // требуют: это тот же код, который автор мог бы принести сам файлом, —
  // просто он уже внутри приложения и работает без интернета.
  'libs.list', 'libs.get',
  ...
  return async function dispatch(method: string, args: unknown[]): Promise<unknown> {
    switch (method) {
      ...
      case 'subscribe': {
        ...
        // v1.491.0: спектр считается только пока его кто-то слушает. Модуль
        // грузится по требованию: у человека без плагинов-визуализаторов он не
        // нужен вовсе, а плеер тянуть в стартовую сборку незачем.
        if (ev === 'audio') void import('../../music/spectrum').then(m => m.watchSpectrum(id))
        return null
      }
      ...
      // ---- Встроенные библиотеки (без разрешения) -------------------------
      //
      // Текст библиотеки уезжает в страницу плагина и выполняется у неё. Ничего
      // о человеке здесь нет вовсе: это код three.js, лежащий в самом
      // приложении. Разрешение за него было бы церемонией — плагин может
      // принести ту же библиотеку своим файлом и без спроса.
      //
      // Грузится по требованию: у человека без плагинов-игр эти сотни
      // килобайт не читаются никогда.
      case 'libs.list': {
        const { libList } = await import('./libs')
        return await libList()
      }
      case 'libs.get': {
        const { libSource } = await import('./libs')
        const b = await libSource(String(args[0] ?? ''))
        if (!b) throw new Denied(`Встроенной библиотеки «${args[0]}» нет. Есть: three.`)
        return b
      }
