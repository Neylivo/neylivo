...
console.log('\n── Сеть ──')
{
  ...
  // v1.445.0: поток (neylivo.net.stream) — второй способ выйти в сеть, появившийся
...
console.log('\n── Оформление ──')
{
  ...
  // непрозрачный слой поверх всего это самый простой способ сделать NeyLivo
  ...
  // простой способ сделать NeyLivo неработающим, при этом «легальными» средствами.
...
console.log('\n── Без границ, но три границы остались (v1.481.0) ──')
{
  ...
  await check('но НЕ к самому NeyLivo — это изоляция сессии, а не «ещё одно правило»', () =>
    checkTarget('https://ponoi.app/api', цель(['*'])) !== null)
  await check('и НЕ к серверу NeyLivo', () =>
...
console.log('\n── Уборка за плагином ──')
{
  ...
  // v1.491.0: прибавился спектр звука — он тоже считается кадрами.
  await check('уборка знает про все двенадцать видов оставленного', () =>
    cleanup.SUBSYSTEM_COUNT === 12)
