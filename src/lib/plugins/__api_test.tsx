...
`)
  ...
  ok('со страницы доступно то же, что из потока', отч.лишнее === 'прошло', String(отч.лишнее))
...
// ── 19. Визуальный редактор сцены (v1.498.0) ───────────────────────────────
//
// Владелец: «не рандомные кубики туда-сюда без объяснения, буквально Unity».
// Здесь проверяется главное обещание этого редактора: сцена, СОБРАННАЯ
// ПОЛЯМИ, правда превращается в игру — с теми объектами, тем цветом и в том
// месте, что задали.
async function редакторСцены(host: Host) {
  const { clearAllApps } = await import('./apps')
  const S = await import('./scene')
  const W = await import('./workshop')
  clearAllApps()

  // Собираем сцену так, как её собрал бы человек: пустая, потом добавили шар и
  // покрасили в приметный цвет.
  let сцена = S.emptyScene()
  const r = S.addNode(сцена, 'sphere')
  сцена = S.updateNode(r.scene, r.id, {
    name: 'Красный шар', color: '#ff3366', pos: [0, 1.2, 0], scale: [2.4, 2.4, 2.4],
    // Скрипт объекта: он обязан работать в игре и молчать в редакторе.
    script: 'function onFrame(dt) { это.rotation.y += dt }',
  })
  // Камеру ставим прямо на шар, чтобы он занял середину кадра.
  сцена = { ...сцена, camera: { pos: [0, 1.2, 4], look: [0, 1.2, 0], fov: 60 }, fly: false }

  ok('сцена собирается полями, а не кодом',
    сцена.nodes.length === 4 && сцена.nodes[3].color === '#ff3366',
    'объектов: ' + сцена.nodes.length)

  const проект = {
    ...W.newSceneProject(), scene: сцена, name: 'Проба сцены', author: 'проба',
    description: 'собрано визуально', width: 320, height: 240,
  }
  const код = W.buildProject(проект as any)
  ok('проект-сцена собирается в плагин', /@id\s+proba-sceny/.test(код))
  ok('сцена сохраняется ДАННЫМИ, а не только собранной страницей',
    код.includes('const СЦЕНА = ') && код.includes('Красный шар'))

  const назад = W.parseProject(код)
  ok('проект открывается обратно сценой, а не простынёй',
    !!назад?.scene && назад.scene.nodes.length === 4
      && назад.scene.nodes[3].script.includes('onFrame'),
    назад?.scene ? 'объектов ' + назад.scene.nodes.length : 'не разобрался')

  // Дописываем отчёт отдельным объектом-пустышкой со скриптом: так же, как это
  // сделал бы человек, а не в обход движка.
  const отчёт = S.makeNode('empty', 'Отчёт')
  отчёт.script = `
// Пиксель читаем ПОСЛЕ отрисовки, а не в onFrame.
//
...
function onStart() {
  ...
  const код2 = W.buildProject({ ...проект, scene: сЛогом } as any)

  const id = await поднять(host, код2)
  ok('игра из сцены открылась окном', await ждать(() => appList(id).length === 1, 8000),
    String(host.pluginError(id) ?? 'без ошибок'))

  await ждать(() => журнал(host, id).some(l => l.includes('СЦЕНА:')), 25000)
  const с = строка(host, id, 'СЦЕНА:')
  let о: any = {}
  try { о = JSON.parse(с.slice('СЦЕНА:'.length)) } catch { /* видно ниже */ }
  const ошибки = журнал(host, id).filter(l => l.startsWith('error:')).join(' | ')

  // Шар красный (#ff3366) и стоит ровно перед камерой: в середине кадра обязан
  // быть он, а не небо. Свет мы не считаем — важно, что красного МНОГО.
  const [кр, зл, си] = String(о.цвет ?? '0,0,0').split(',').map(Number)
  ok('в середине кадра — заданный объект, а не пустое небо',
    кр > 60 && кр > зл * 2 && кр > си, 'цвет: ' + о.цвет + ' | ' + (ошибки || 'ошибок нет'))
  ok('скрипт объекта выполнился', !!с, ошибки.slice(0, 160))
  ok('ошибок в сцене нет', !ошибки, ошибки.slice(0, 200))
...
async function main() {
  ...
  lines.push(''); lines.push('── Редактор сцены ──'); out()
  await редакторСцены(host)
