...
async function main() {
  ...
  for (let i = 0; i < MAX_ASSETS; i++) await assetPut('probe', 'f' + i + '.json', '{"n":' + i + '}')
  await mustThrow('за предел числа файлов не пускает', () => assetPut('probe', 'лишний.json', '{}'))
  ok('перезаписать существующий у предела всё равно можно',
