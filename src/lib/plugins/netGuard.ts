...
// никогда не к самому NeyLivo и его серверу, белый список заголовков) жили внутри
 ...
 * остальное — объявленные домены, запрет на самого NeyLivo и его сервер — обязано
...
export function checkTarget(rawUrl: string, t: NetTarget, scheme: 'https:' | 'wss:' = 'https:'): string | null {
  ...
  // ограничение», а граница безопасности: запрос к самому NeyLivo и к его
  ...
  if (host === t.selfHost.toLowerCase() || (t.supaHost && host === t.supaHost.toLowerCase())) {
    return 'Обращаться к самому NeyLivo и его серверу плагинам нельзя.'
  }
  return null
}
