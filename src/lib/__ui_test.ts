...
// Та же строка стоит в scripts/clip-test.cjs: имя клипа собирается ещё и в
// electron/clipRecorder.cjs (для F7), и обе копии обязаны совпадать.
check('формат имени тот же, что у горячей клавиши', () =>
  clipName(new Date(2026, 7, 5, 12, 0, 0), 'Half-Life 2') === 'Half-Life 2 2026-08-05 12-00-00.webm')
...
// выбора умел удалять и пересылать; скопировать кусок разговора приходилось
// мышью по одному сообщению.
console.log('\n-- Копирование выбранного --')

const лента2 = [
  { id: 'a', author_name: 'Ваня', content: 'привет' },
  { id: 'b', author_name: 'я', content: 'здорово' },
  { id: 'c', author_name: 'Ваня', content: 'как дела' },
  { id: 'd', author_name: 'я', content: '', attach_url: 'https://x/файл.zip' },
  { id: 'e', author_name: 'я', content: '   ' },
]
const наб2 = (...ids: string[]) => new Set(ids)

check('копируется «Имя: текст» по строке на сообщение', () =>
  собратьТекст(наб2('a', 'b'), лента2) === 'Ваня: привет' + String.fromCharCode(10) + 'я: здорово')
check('порядок ЛЕНТЫ, а не нажатий', () =>
  // Выбрали снизу вверх — скопировалось всё равно как читается.
  собратьТекст(наб2('c', 'a'), лента2) === 'Ваня: привет' + String.fromCharCode(10) + 'Ваня: как дела')
check('вложение без текста не превращается в пустую строку', () =>
  собратьТекст(наб2('d'), лента2) === 'я: [вложение]')
check('сообщение из одних пробелов не оставляет дыру', () =>
  собратьТекст(наб2('a', 'e'), лента2) === 'Ваня: привет')
check('ничего не выбрано — пусто, а не пустая строка с именем', () =>
  собратьТекст(наб2(), лента2) === '')
check('число на кнопке совпадает с тем, что уйдёт в буфер', () =>
  copyCount(наб2('a', 'b', 'e'), лента2) === 2
  && copyCount(наб2(), лента2) === 0
  && copyCount(наб2('a', 'b'), лента2) === собратьТекст(наб2('a', 'b'), лента2).split(String.fromCharCode(10)).length)

console.log('\n-- Ломаем нарочно (копирование) --')
check('проверка ловит копирование в порядке нажатий', () => {
  const плохо = 'Ваня: как дела' + String.fromCharCode(10) + 'Ваня: привет'
  return плохо !== собратьТекст(наб2('c', 'a'), лента2)
})
check('проверка ловит пустые строки от вложений', () => {
  const плохо = 'я: '
  return плохо !== собратьТекст(наб2('d'), лента2)
})


// -- v1.531.0: длина сообщения ничего не выдаёт ------------------------------
//
// Владелец: «расшифровка даже не знает, отправлен ли один символ или тысяча».
// Здесь проверяется САМ БЛОК, отдельно от шифрования: что длина не видна, что
// текст возвращается целым и что старые сообщения не потерялись.
console.log('\n-- Скрытие длины сообщения --')

// Шум подставной: проверке важно не качество случайности (её даёт система), а
// то, что блок заполнен целиком и текст достаётся обратно.
const шум = (n: number) => new Uint8Array(n).fill(0x5a)

check('«да» и «нет» дают блок одного размера', () =>
  pad('да', шум).length === pad('нет', шум).length)
check('короткое и полкилобайта — тоже', () =>
  pad('да', шум).length === pad('я'.repeat(400), шум).length)
check('текст достаётся обратно ровно тем же', () => {
  const пробы = ['да', '', 'строка' + String.fromCharCode(10) + 'вторая', 'эмодзи 🐾', 'x'.repeat(3000)]
  return пробы.every(т => unpad(pad(т, шум)) === т)
})
check('свободное место засыпано, а не оставлено нулями', () => {
  const б = pad('да', шум)
  // Хвост блока — это шум, а не нули: нули сжались бы, и длина снова стала бы
  // видна тому, кто применит сжатие к шифротексту.
  return б[б.length - 1] === 0x5a && б[б.length - 100] === 0x5a
})
check('ступень выбирается по длине, а не наугад', () =>
  stepFor(10) === STEPS[0] && stepFor(STEPS[0]) === STEPS[1]
  && stepFor(STEPS[0] - 100) === STEPS[0])
check('очень длинное укладывается в мегабайты, а не отбрасывается', () =>
  stepFor(3 * 1024 * 1024) % (1024 * 1024) === 0 && stepFor(3 * 1024 * 1024) >= 3 * 1024 * 1024)

check('старое сообщение без блока читается как раньше', () =>
  // Иначе вся переписка до обновления исчезла бы с экрана.
  unpad(new TextEncoder().encode('привет из прошлой версии')) === 'привет из прошлой версии')
check('испорченная длина не выдаёт чужой шум за текст', () => {
  const б = pad('да', шум)
  б[3] = 0xff; б[4] = 0xff   // длина больше блока
  return unpad(б) === ''
})

console.log('\n-- Ломаем нарочно (длина) --')
check('проверка ловит возврат «длина как у текста»', () => {
  const плохо = (t: string) => new TextEncoder().encode(t)
  return плохо('да').length !== плохо('нет').length
    && pad('да', шум).length === pad('нет', шум).length
})
...
//
// Владелец: «если сообщение написано вчера или позавчера, вместо даты писать
// вчера или позавчера и время». До этого вчерашнее подписывалось «5 авг.,
// 21:13» — число надо было сравнивать с сегодняшним.
//
// Главное здесь не сами слова, а СЧЁТ ДНЕЙ. Считать разницу во времени и делить
// на сутки — привычная ошибка: сообщение вчера в 23:50 через двадцать минут
// стало бы «сегодняшним», а написанное двадцать часов назад — «вчерашним»,
// хотя календарно оно позавчерашнее. Поэтому ниже границы полуночи проверяются
// отдельно.
console.log('\n-- Вчера и позавчера --')
setTime24(true)

// Полдень 6 августа 2026 — от него и считаем.
const ПОЛДЕНЬ = new Date(2026, 7, 6, 12, 0)
const вт = (д: number, ч: number, м = 0) => new Date(2026, 7, д, ч, м).toISOString()

check('сегодняшнее — просто время', () => msgTime(вт(6, 9, 5), ПОЛДЕНЬ) === '09:05')
check('вчерашнее — словом и временем', () => msgTime(вт(5, 21, 13), ПОЛДЕНЬ) === 'Вчера в 21:13')
check('позавчерашнее — тоже словом', () => msgTime(вт(4, 8, 30), ПОЛДЕНЬ) === 'Позавчера в 08:30')
check('три дня назад — снова дата', () => {
  const t = msgTime(вт(3, 8, 30), ПОЛДЕНЬ)
  return t.includes('авг') && t.includes('08:30') && !t.includes('Позавчера')
})

check('вчера в 23:50, а сейчас 00:10 — всё ещё «Вчера»', () =>
  // Прошло двадцать минут. Мерка «сколько миллисекунд назад» сказала бы
  // «сегодня», и подпись разошлась бы с разделителем дня в ленте.
  msgTime(вт(5, 23, 50), new Date(2026, 7, 6, 0, 10)) === 'Вчера в 23:50')
check('двадцать часов назад бывает позавчерашним', () =>
  // 4 августа 23:00, сейчас 5 августа 19:00 — прошло двадцать часов, но по
  // календарю это вчера, а не сегодня.
  msgTime(вт(4, 23, 0), new Date(2026, 7, 5, 19, 0)) === 'Вчера в 23:00')
check('сегодня в 00:01 — это сегодня, а не вчера', () =>
  msgTime(вт(6, 0, 1), ПОЛДЕНЬ) === '00:01')
check('сообщение из будущего не становится вчерашним', () =>
  // Сбитые часы на устройстве: пишем время вперёд. «Вчера» тут было бы ложью.
  msgTime(вт(7, 10, 0), ПОЛДЕНЬ) === '10:00')
check('мусор вместо времени не рисует «Invalid Date»', () =>
  msgTime('не время', ПОЛДЕНЬ) === '' && dayLabel('не время', ПОЛДЕНЬ) === '')

check('разделитель дня знает те же три слова', () =>
  dayLabel(вт(6, 1, 0), ПОЛДЕНЬ) === 'Сегодня'
  && dayLabel(вт(5, 1, 0), ПОЛДЕНЬ) === 'Вчера'
  && dayLabel(вт(4, 1, 0), ПОЛДЕНЬ) === 'Позавчера')
check('и дальше третьего дня снова дата', () =>
  dayLabel(вт(3, 1, 0), ПОЛДЕНЬ).includes('август'))
check('прошлый год подписывается годом', () =>
  msgTime(new Date(2025, 7, 6, 12, 0).toISOString(), ПОЛДЕНЬ).includes('2025'))

check('дни считаются календарём, а не сутками', () =>
  daysAgo(new Date(2026, 7, 5, 23, 59), new Date(2026, 7, 6, 0, 1)) === 1
  && daysAgo(new Date(2026, 7, 6, 0, 1), new Date(2026, 7, 6, 23, 59)) === 0
  && daysAgo(new Date(2026, 6, 31, 12, 0), new Date(2026, 7, 2, 12, 0)) === 2)

console.log('\n-- Ломаем нарочно (вчера и позавчера) --')
check('проверка ловит счёт по суткам вместо календаря', () => {
  // Так и ошибаются: «прошло меньше суток — значит сегодня».
  const плохо = (когда: Date, сейчас: Date) => Math.floor((+сейчас - +когда) / 86400000)
  return плохо(new Date(2026, 7, 5, 23, 50), new Date(2026, 7, 6, 0, 10)) === 0
    && daysAgo(new Date(2026, 7, 5, 23, 50), new Date(2026, 7, 6, 0, 10)) === 1
})
check('проверка ловит потерянное время рядом со словом', () =>
  // «Вчера» без времени — это меньше, чем было до правки.
  msgTime(вт(5, 21, 13), ПОЛДЕНЬ) !== 'Вчера' && msgTime(вт(5, 21, 13), ПОЛДЕНЬ).includes('21:13'))
...
{
  const { seen, seenLabel } = await import('./dmReads')
  const метка = (at: number) => ({ threadId: 't', userId: 'u', at })
  const час = (h: number, m = 0) => { const d = new Date(); d.setHours(h, m, 0, 0); return d.getTime() }

  check('без отметки сообщение не считается прочитанным', () =>
    !seen(Date.now(), null) && seenLabel(Date.now(), null) === 'Доставлено')

  check('прочитано, если собеседник дочитал ПОЗЖЕ сообщения', () =>
    seen(час(12, 0), метка(час(12, 5))) && !seen(час(12, 10), метка(час(12, 5))))

  check('секунда допуска: сообщение и отметка приходят почти одновременно', () => {
    // Время ставит сервер обеим записям, и без допуска только что прочитанное
    // выглядело бы непрочитанным.
    const t = Date.now()
    return seen(t, метка(t - 300)) && !seen(t, метка(t - 5000))
  })

  check('строка времени человеческая, а не машинная', () => {
    const s = seenLabel(час(9, 0), метка(час(9, 30)))
    return s.startsWith('Просмотрено в ') && /\d/.test(s)
  })

  check('вчерашнее прочтение показывает и дату', () => {
    const вчера = Date.now() - 26 * 3600 * 1000
    return seenLabel(вчера - 1000, метка(вчера)).includes('Просмотрено ')
      && seenLabel(вчера - 1000, метка(вчера)).split(' ').length > 3
  })

  check('битое время не роняет разбор и не врёт', () =>
...
check('кусок меньше предела хранилища', () => КУСОК <= 45 * 1024 * 1024)
check('маленький файл не режется', () => !надоРезать(1024) && !надоРезать(КУСОК))
check('файл больше куска режется', () => надоРезать(КУСОК + 1))
check('число кусков считается вверх', () =>
  сколькоКусков(КУСОК) === 1 && сколькоКусков(КУСОК + 1) === 2
  && сколькоКусков(КУСОК * 3) === 3 && сколькоКусков(0) === 0)
// Границы обязаны покрыть файл без дыр и без нахлёста: дыра — это битое видео,
// нахлёст — лишние байты внутри, и оба видны только по испорченному файлу.
check('границы кусков покрывают файл ровно один раз', () => {
  const размер = КУСОК * 2 + 12345
  let собрано = 0, прошлыйКонец = 0
  for (let n = 0; n < сколькоКусков(размер); n++) {
    const { от, до } = границы(размер, n)
    if (от !== прошлыйКонец) return false
    if (до <= от) return false
    собрано += до - от
    прошлыйКонец = до
  }
  return собрано === размер && прошлыйКонец === размер
})
check('последний кусок короче остальных', () => {
  const размер = КУСОК + 100
  const п = границы(размер, 1)
  return п.до - п.от === 100
})
check('имена кусков сортируются по порядку', () => {
  const имена = [0, 1, 2, 10, 11, 100].map(имяКуска)
  return имена.join(',') === [...имена].sort().join(',')
})
check('опись узнаётся по адресу', () =>
  этоОпись('https://x/y/z.ponoipart')
  && этоОпись('https://x/y/z.ponoipart?token=1')
  && !этоОпись('https://x/y/z.mp4')
  && !этоОпись(''))

const хост = 'x.supabase.co'
const годная = опись('клип.webm', 'video/webm', КУСОК + 10,
  ['https://x.supabase.co/a', 'https://x.supabase.co/b'])
check('своя опись принимается', () => годнаяОпись(годная, хост))
// Опись приходит из сети, то есть это данные извне. По списку адресов
// приложение пойдёт скачивать — подсунуть туда чужой хост нельзя.
check('опись с чужим хостом отвергается', () =>
  !годнаяОпись(опись('a', 'video/webm', КУСОК + 10,
    ['https://x.supabase.co/a', 'https://зло.example/b']), хост))
check('опись с не-http адресом отвергается', () =>
  !годнаяОпись(опись('a', 'v', КУСОК + 10, ['javascript:alert(1)', 'https://x.supabase.co/b']), хост))
check('опись, где размер не сходится с числом кусков, отвергается', () =>
  !годнаяОпись(опись('a', 'v', КУСОК * 10, ['https://x.supabase.co/a']), хост))
check('опись без кусков отвергается', () =>
  !годнаяОпись(опись('a', 'v', 100, []), хост)
  && !годнаяОпись({ v: 1, name: 'a', type: 'v', size: 100 }, хост)
  && !годнаяОпись(null, хост)
  && !годнаяОпись('строка', хост))
check('опись другой версии отвергается', () =>
  !годнаяОпись({ ...годная, v: 2 }, хост))

check('размер пишется по-человечески', () =>
  размерСловами(900) === '900 Б' && размерСловами(2 * 1024 ** 2) === '2,0 МБ'
  && размерСловами(3 * 1024 ** 3) === '3,00 ГБ')

// Переход на куски должен случаться ТОЛЬКО от отказа по размеру: повторять
// кусками отказ по правам — значит тридцать раз получить тот же отказ.
check('отказ по размеру узнаётся', () =>
  отказПоРазмеру(new Error('Загрузка не удалась (413) Payload too large'))
