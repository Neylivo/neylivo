...
export function AuthScreen() {
  ...
  async function afterLoginKeys(pwd: string) {
    try {
      ...
      // v1.559.0: копия под своим паролем НЕ перезаписывается паролем аккаунта.
      // Ключ спросим отдельно, уже в приложении, — см. lib/keyPrompt.ts.
      if (res === 'own-password') { localStorage.setItem('ponoi-key-ask', '1'); return }
