...
//
// ВЫКЛЮЧЕНО по умолчанию, включается переключателем в настройках (v1.556.1,
...
let captureGuard = (() => {
  try { return readPrefs().captureGuard === true } catch { return false }
})()
...
function dotaMode() {
  ...
  const d = lastGsi.data
  const stRaw = d.map ? d.map.game_state : null
  ...
  if (stRaw === 'DOTA_GAMERULES_STATE_POST_GAME') {
    ...
    const my = String((d.player && d.player.team_name) || '').toLowerCase()
    if ((win === 'radiant' || win === 'dire') && (my === 'radiant' || my === 'dire')) {
      const res = win === my ? 'Победа' : 'Поражение'
...
function dbdMode() {
  try {
    ...
    if (mm.length) {
      ...
      // v1.132.0: если последняя загруженная «карта» — меню, значит игрок в лобби.
      if (raw && /^(menu|lobby|offline|frontend)$/i.test(raw)) return 'В лобби'
      if (raw) map = raw
    }
...
// карт. Официального API у этих игр нет — это best-effort, как DBD: перестанут
// писать лог — просто не покажем деталей (сама игра в статусе останется).
function readLogTail(f, cap) {
  try {
    const fsr = require('fs')
    if (!fsr.existsSync(f)) return null
    const size = fsr.statSync(f).size
    const len = Math.min(size, cap || 256 * 1024)
    const buf = Buffer.alloc(len)
    const fd = fsr.openSync(f, 'r')
    fsr.readSync(fd, buf, 0, len, size - len)
    fsr.closeSync(fd)
...
// Стандартные состояния матча Unreal + специфичные для Fortnite (Aircraft — автобус).
const UE_STATES = { waitingtostart: 'В лобби матча', aircraft: 'В автобусе', safezones: 'В матче',
  inprogress: 'В матче', waitingpostmatch: 'Конец матча', leavingmap: 'Конец матча' }
function ueLogMode(logFile) {
  const txt = readLogTail(logFile)
  if (!txt) return null
  let stAt = -1, st = null
  const re = /Match State Changed from (\w+) to (\w+)/g
  let m
  while ((m = re.exec(txt))) { stAt = m.index; st = m[2] }
  // Возврат в главное меню (Frontend) после последней смены состояния — это лобби.
  const feAt = Math.max(txt.lastIndexOf('Frontend'), txt.lastIndexOf('frontend'))
  if (feAt > stAt) return 'В лобби'
  return st ? (UE_STATES[st.toLowerCase()] || null) : null
}
